'use client';

import { useEffect, useMemo, useState } from 'react';
import Link from 'next/link';
import axios from 'axios';
import { getAuthHeaders } from '@/lib/authHeaders';

export default function StructureSettingsPage() {
  const backendUrl = process.env.NEXT_PUBLIC_API_BASE_URL;

  const [structureOptions, setStructureOptions] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);

  const [search, setSearch] = useState('');
  const [typeFilter, setTypeFilter] = useState('ALL');
  const [sortBy, setSortBy] = useState('capacity-asc');
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [page, setPage] = useState(1);

  const [form, setForm] = useState({
    structureType: '',
    capacityKw: '',
    ratePerKw: '',
  });

  const [editingId, setEditingId] = useState<number | null>(null);
  const [editingData, setEditingData] = useState<any>({});

  const fetchOptions = async () => {
    try {
      setLoading(true);

      const res = await axios.get(`${backendUrl}/calculator/structure-options`, {
        headers: getAuthHeaders(),
      });

      setStructureOptions(res.data || []);
    } catch (err) {
      console.error(err);
      alert('Failed to load structure options');
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchOptions();
  }, []);

  const filteredOptions = useMemo(() => {
    let list = [...structureOptions];

    if (typeFilter !== 'ALL') {
      list = list.filter((item) => item.structureType === typeFilter);
    }

    if (search.trim()) {
      const q = search.toLowerCase();

      list = list.filter((item) =>
        `${item.structureType} ${item.capacityKw} ${item.ratePerKw}`
          .toLowerCase()
          .includes(q)
      );
    }

    list.sort((a, b) => {
      if (sortBy === 'capacity-asc') {
        return Number(a.capacityKw) - Number(b.capacityKw);
      }

      if (sortBy === 'capacity-desc') {
        return Number(b.capacityKw) - Number(a.capacityKw);
      }

      if (sortBy === 'rate-asc') {
        return Number(a.ratePerKw) - Number(b.ratePerKw);
      }

      if (sortBy === 'rate-desc') {
        return Number(b.ratePerKw) - Number(a.ratePerKw);
      }

      return 0;
    });

    return list;
  }, [structureOptions, search, typeFilter, sortBy]);

  const totalPages = Math.max(1, Math.ceil(filteredOptions.length / rowsPerPage));

  const paginatedOptions = filteredOptions.slice(
    (page - 1) * rowsPerPage,
    page * rowsPerPage
  );

  useEffect(() => {
    setPage(1);
  }, [search, typeFilter, sortBy, rowsPerPage]);

  const createOption = async () => {
    if (!form.capacityKw || !form.ratePerKw) {
      alert('Please enter capacity and rate');
      return;
    }

    try {
      await axios.post(
        `${backendUrl}/calculator/structure-options`,
        {
          structureType: form.structureType,
          capacityKw: Number(form.capacityKw),
          ratePerKw: Number(form.ratePerKw),
        },
        { headers: getAuthHeaders() }
      );

      setForm({
        structureType: '',
        capacityKw: '',
        ratePerKw: '',
      });

      fetchOptions();
    } catch (err) {
      console.error(err);
      alert('Create failed');
    }
  };

  const deleteOption = async (id: number) => {
    if (!confirm('Delete this structure option?')) return;

    try {
      await axios.delete(`${backendUrl}/calculator/structure-options/${id}`, {
        headers: getAuthHeaders(),
      });

      fetchOptions();
    } catch (err) {
      console.error(err);
      alert('Delete failed');
    }
  };

  const updateOption = async () => {
    if (!editingId) return;

    try {
      await axios.patch(
        `${backendUrl}/calculator/structure-options/${editingId}`,
        {
          structureType: editingData.structureType,
          capacityKw: Number(editingData.capacityKw),
          ratePerKw: Number(editingData.ratePerKw),
        },
        { headers: getAuthHeaders() }
      );

      setEditingId(null);
      setEditingData({});
      fetchOptions();
    } catch (err) {
      console.error(err);
      alert('Update failed');
    }
  };

  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">Structure Items</h1>
          <p className="text-sm text-gray-500">
            Manage Rooftop / Tin Shade capacity and rate per kW.
          </p>
        </div>

        <Link
          href="/calculator/settings"
          className="text-sm bg-gray-100 px-4 py-2 rounded-lg"
        >
          Back to Settings
        </Link>
      </div>

      <div className="bg-white p-5 rounded-2xl shadow space-y-4">
        <h2 className="font-semibold">Add New Structure Item</h2>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <input
  placeholder="Structure Type (e.g. Ground Mount)"
  value={form.structureType}
  onChange={(e) =>
    setForm({ ...form, structureType: e.target.value })
  }
  className="border p-2 rounded"
/>

          <input
            type="number"
            placeholder="Capacity kW"
            value={form.capacityKw}
            onChange={(e) => setForm({ ...form, capacityKw: e.target.value })}
            className="border p-2 rounded"
          />

          <input
            type="number"
            placeholder="Rate per kW"
            value={form.ratePerKw}
            onChange={(e) => setForm({ ...form, ratePerKw: e.target.value })}
            className="border p-2 rounded"
          />

          <button
            onClick={createOption}
            className="bg-green-600 text-white rounded px-4 py-2"
          >
            Add
          </button>
        </div>
      </div>

      <div className="bg-white p-5 rounded-2xl shadow space-y-4">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            <h2 className="font-semibold">Saved Structure Items</h2>
            <p className="text-sm text-gray-500">
              Showing {paginatedOptions.length} of {filteredOptions.length} filtered options.
              Total saved: {structureOptions.length}
            </p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <input
            placeholder="Search type, capacity or rate"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="border p-2 rounded"
          />

          <select
            value={typeFilter}
            onChange={(e) => setTypeFilter(e.target.value)}
            className="border p-2 rounded"
          >
            <option value="ALL">All Types</option>
            {Array.from(new Set(structureOptions.map((o) => o.structureType))).map(
  (type) => (
    <option key={type} value={type}>
      {type}
    </option>
  )
)}
          </select>

          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value)}
            className="border p-2 rounded"
          >
            <option value="capacity-asc">Capacity: Low to High</option>
            <option value="capacity-desc">Capacity: High to Low</option>
            <option value="rate-asc">Rate: Low to High</option>
            <option value="rate-desc">Rate: High to Low</option>
          </select>

          <select
            value={rowsPerPage}
            onChange={(e) => setRowsPerPage(Number(e.target.value))}
            className="border p-2 rounded"
          >
            <option value={10}>10 rows</option>
            <option value={25}>25 rows</option>
            <option value={50}>50 rows</option>
          </select>
        </div>

        {loading ? (
          <div>Loading...</div>
        ) : paginatedOptions.length === 0 ? (
          <div className="border rounded p-4 text-gray-500">
            No structure options found.
          </div>
        ) : (
          <div className="space-y-2">
            {paginatedOptions.map((opt) => (
              <div
                key={opt.id}
                className="border p-3 rounded flex flex-col md:flex-row md:items-center md:justify-between gap-3"
              >
                {editingId === opt.id ? (
                  <>
                    <input
  placeholder="Structure Type"
  value={editingData.structureType || ''}
  onChange={(e) =>
    setEditingData({
      ...editingData,
      structureType: e.target.value,
    })
  }
  className="border p-2 rounded"
/>
                    <input
                      type="number"
                      value={editingData.capacityKw}
                      onChange={(e) =>
                        setEditingData({
                          ...editingData,
                          capacityKw: e.target.value,
                        })
                      }
                      className="border p-2 rounded"
                    />

                    <input
                      type="number"
                      value={editingData.ratePerKw}
                      onChange={(e) =>
                        setEditingData({
                          ...editingData,
                          ratePerKw: e.target.value,
                        })
                      }
                      className="border p-2 rounded"
                    />

                    <div className="space-x-3">
                      <button onClick={updateOption} className="text-green-600">
                        Save
                      </button>

                      <button
                        onClick={() => {
                          setEditingId(null);
                          setEditingData({});
                        }}
                        className="text-gray-600"
                      >
                        Cancel
                      </button>
                    </div>
                  </>
                ) : (
                  <>
                    <div className="text-sm">
                      <div className="font-medium">{opt.structureType}</div>
                      <div className="text-gray-600">
                        {opt.capacityKw} kW | ₹{opt.ratePerKw} per kW
                      </div>
                    </div>

                    <div className="space-x-3">
                      <button
                        onClick={() => {
                          setEditingId(opt.id);
                          setEditingData({ ...opt });
                        }}
                        className="text-blue-600"
                      >
                        Edit
                      </button>

                      <button
                        onClick={() => deleteOption(opt.id)}
                        className="text-red-600"
                      >
                        Delete
                      </button>
                    </div>
                  </>
                )}
              </div>
            ))}
          </div>
        )}

        <div className="flex items-center justify-between pt-3">
          <button
            disabled={page <= 1}
            onClick={() => setPage((prev) => Math.max(1, prev - 1))}
            className="px-4 py-2 rounded bg-gray-100 disabled:opacity-50"
          >
            Previous
          </button>

          <span className="text-sm text-gray-600">
            Page {page} of {totalPages}
          </span>

          <button
            disabled={page >= totalPages}
            onClick={() => setPage((prev) => Math.min(totalPages, prev + 1))}
            className="px-4 py-2 rounded bg-gray-100 disabled:opacity-50"
          >
            Next
          </button>
        </div>
      </div>
    </div>
  );
}