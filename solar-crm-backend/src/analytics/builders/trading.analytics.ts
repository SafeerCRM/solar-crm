...
export class TradingAnalyticsBuilder {
  ...
  const tradingOutstanding = Math.max(dealerOrderAmount - approvedDealerPayments, 0);
  ...
  return {
    ...
    cards: {
      ...
      dealerOrderAmount,
      dealerOrderPaidAmount: Number(orderSummary?.dealerOrderPaidAmount || 0),
      dealerOrderPendingAmount: Number(orderSummary?.dealerOrderPendingAmount || 0),
